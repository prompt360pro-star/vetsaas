...
export const Input = forwardRef<HTMLInputElement, InputProps>(
    ({ label, error, icon, hint, className = '', id: propId, ...props }, ref) => {
        ...
        const id = propId || `input-${fallbackId}`;
        const errorId = `error-${fallbackId}`;
        const hintId = `hint-${fallbackId}`;
        ...
        return (
            <div className="w-full">
                ...
                <div className="relative">
                    {icon && (
                        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-surface-400 pointer-events-none" aria-hidden="true">
                            {icon}
                        </span>
                    )}
                    <input
