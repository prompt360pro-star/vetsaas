...
export function Select({ label, error, options, placeholder, className = '', id: propsId, ...props }: SelectProps) {
    const generatedId = useId();
    const id = propsId || generatedId;
    const errorId = error ? `${id}-error` : undefined;
    const ariaDescribedBy = [errorId].filter(Boolean).join(' ') || undefined;
