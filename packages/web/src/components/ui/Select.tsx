...
export function Select({ label, error, options, placeholder, hint, className = '', id: propId, ...props }: SelectProps) {
    const fallbackId = useId();
    const selectId = propId || fallbackId;
    ...
    return (
        <div className="w-full">
            ...
            {error && <p id={errorId} className="mt-1.5 text-xs text-danger flex items-center gap-1">{error}</p>}
            {hint && !error && <p id={hintId} className="mt-1.5 text-xs text-surface-400">{hint}</p>}
        </div>
    );
}
