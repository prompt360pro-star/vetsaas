...
export class GenericSmsProvider implements ISmsProvider {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.bodyTemplate = this.configService.get<string>(
      "SMS_GENERIC_BODY_TEMPLATE",
      "",
    );
    ...
    if (!this.apiUrl) {
      this.logger.warn(
        "Generic SMS configuration is missing (URL). SMS sending will fail if Generic provider is selected.",
      );
    }
  }
  ...
  async send(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `Failed to send SMS via Generic provider: ${response.status} ${response.statusText} - ${errorText}`,
        );
        return {
          success: false,
      ...
      this.logger.error(
        `Error sending SMS via Generic provider: ${error.message}`,
        error.stack,
      );
      return {
        success: false,
