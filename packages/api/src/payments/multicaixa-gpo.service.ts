...
export class MulticaixaGpoService {
    ...
    constructor(private readonly config: ConfigService) {
        ...
        this.merchantId = this.config.get<string>('MULTICAIXA_GPO_MERCHANT_ID', '00000');
    }

    async generateReference(amount: number): Promise<string> {
        // If credentials are missing, fallback immediately
        if (!this.apiUrl || !this.apiKey) {
            this.logger.warn('Multicaixa GPO API URL or Key not configured. Using fallback reference.');
        ...
        try {
            ...
            this.logger.error(`Failed to generate Multicaixa reference: ${error.message}`);
