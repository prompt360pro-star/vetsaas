...
export class PaymentsService {
  ...
  private async handleMulticaixaWebhook(
    payload: Record<string, unknown>,
  ): Promise<void> {
    ...
    if (status === "00" || status === "SUCCESS") {
      ...
      this.logger.log(
        `[WEBHOOK] Payment ${payment.id} completed via Multicaixa GPO`,
      );
    } else {
  ...
  private async handleUnitelMoneyWebhook(
    payload: Record<string, unknown>,
  ): Promise<void> {
    ...
    if (status === "PAID") {
      ...
      await this.repo.save(payment);
      this.logger.log(
        `[WEBHOOK] Payment ${payment.id} completed via Unitel Money`,
      );
      ...
      await this.repo.save(payment);
      this.logger.log(
        `[WEBHOOK] Payment ${payment.id} failed via Unitel Money`,
      );
    } else {
      this.logger.warn(
  ...
  private async handleGenericWebhook(
    ...
    this.logger.log(
      `[WEBHOOK] Payment ${payment.id} marked as COMPLETED (Generic)`,
    );
  }
