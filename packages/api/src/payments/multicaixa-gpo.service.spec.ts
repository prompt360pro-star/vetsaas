...
describe('MulticaixaGpoService', () => {
                ...
                MulticaixaGpoService,
                {
                    ...
                    useValue: {
                        get: jest.fn((key: string, defaultValue: any) => {
                            if (key === 'MULTICAIXA_GPO_API_URL') return 'http://mock-api.com';
    ...
    it('should generate reference using API', async () => {
        ...
        expect(global.fetch).toHaveBeenCalledWith('http://mock-api.com/references', expect.objectContaining({
            method: 'POST',
            body: JSON.stringify({ amount: 1000, merchantId: '12345', currency: 'AOA' }),
        }));
    });
    ...
    it('should fallback if config is missing', async () => {
        ...
        const serviceNoConfig = module.get<MulticaixaGpoService>(MulticaixaGpoService);
