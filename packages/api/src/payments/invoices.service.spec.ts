...
describe("InvoicesService", () => {
  ...
  describe("markAsPaid", () => {
    it("should mark invoice as paid and link payment", async () => {
      ...
      const _result = await service.markAsPaid(
        tenantId,
        "inv-uuid-1",
        "pay-uuid-1",
      );
  ...
  describe("cancel", () => {
    it("should cancel a draft invoice", async () => {
      ...
      const _result = await service.cancel(
        tenantId,
        "inv-uuid-1",
        "Erro de dados",
      );
    ...
    it("should reject cancelling a paid invoice", async () => {
      ...
      await expect(service.cancel(tenantId, "inv-uuid-1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("send", () => {
    it("should send a draft invoice", async () => {
      ...
      const _result = await service.send(tenantId, "inv-uuid-1");
